  ...
  if (isset($_POST['submit']))
   {
      ...
      $email = $_POST['mail'];
      $assigned_hospital = $_POST['selected'];

      
      $insertion = "INSERT INTO user_registration (username,contact,email,hospital_id) VALUES ('$username','$contact','$email','$assigned_hospital')";
      $query = mysqli_query($connect, $insertion);

      $insertion2 = "INSERT INTO user_login (email,hospital_id) VALUES ('$email','$assigned_hospital')";
      $query2 = mysqli_query($connect, $insertion2);
      ...
      if ($query && $query2)
       {
            ...
            <center><img src='images/success.png' height='100' width='100'>
            <h2>".$username.' with email address '.$email." has successfully registered</h2>
            </center>
            </div>
            </div>
          </header>";
          header("refresh:3; url=register_users.php");
       }

       else
       {
        header('location:register_users.php');
        echo "could not register".$username;
       }
